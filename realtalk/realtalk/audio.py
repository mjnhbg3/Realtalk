...
class PCMQueueAudioSource(discord.AudioSource):
    ...
    def __init__(
        ...
        max_queue_size: int = 15,  # ~300ms of audio - balanced to prevent drops and timing compensation
        volume: float = 1.0
    ):
        super().__init__()
        ...
        self.min_buffer_size = 4  # Minimum frames before allowing playback start (80ms)
            ...
            with self.queue_lock:
                ...
                while idx + frame_bytes <= buflen:
                    ...
                    if len(self.audio_queue) >= self.max_queue_size:
                        # Use a hybrid approach: drop oldest but limit frequency to prevent timing issues
                        if len(self.audio_queue) >= self.max_queue_size + 2:  # Only drop if significantly over
                            old_frame = self.audio_queue.popleft()
                            self.frames_dropped += 1
                            if self.frames_dropped % 20 == 1:
                                log.debug(f"Dropped oldest frame: queue was {len(self.audio_queue)} frames")
                        else:
                            # Near limit but not over - skip this frame to prevent buffer growth
                            self.frames_dropped += 1
                            if self.frames_dropped % 20 == 1:
                                log.debug(f"Skipped frame: queue at {len(self.audio_queue)} frames")
                            continue
    ...
    def _to_48k_stereo(self, audio_data: bytes) -> bytes:
        ...
        try:
            ...
            # Convert to numpy array (keeping as float32 for interpolation)
