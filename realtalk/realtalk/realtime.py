...
class RealtimeClient:
    ...
    def __init__(self, api_key: str, model: Optional[str] = None, voice: Optional[str] = None, transcribe: Optional[str] = None, server_vad: Optional[Dict[str, Any]] = None, instructions: Optional[str] = None):
        ...
        self.on_input_audio_transcript: Optional[Callable[[str], None]] = None
    ...
    async def commit_audio_buffer(self):
        ...
        if self._buffered_ms < 100.0:
            log.debug(f"Skipping commit - insufficient audio ({self._buffered_ms:.2f}ms < 100ms)")
            return
            
        try:
            await self._send_message({"type": "input_audio_buffer.commit"})
            self._buffered_ms = 0.0
            log.debug(f"Successfully committed audio buffer")
        except Exception as e:
    ...
    async def _handle_event(self, event: Dict[str, Any]):
        ...
        try:
            if event_type == "session.created":
                ...
                try:
                    await self.commit_audio_buffer()
                    if self._buffered_ms == 0.0:  # Successfully committed
                        await self.create_response()
                except Exception as e:
                ...
                log.debug(f"Input audio transcription: {transcript}")
                if self.on_input_audio_transcript:
                    self.on_input_audio_transcript(transcript)
