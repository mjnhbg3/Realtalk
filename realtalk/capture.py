...
Enhanced audio capture with voice-recv integration, noise filtering,
...
class VoiceCapture:
    ...
    - Improved voice-recv integration with better error handling
    - Enhanced segmented recording with noise filtering
    - Better audio mixing and speaker selection
    ...
    def __init__(
        ...
        self.sink: Optional[voice_recv.AudioSink] = None
    ...
    def _recording_finished_callback(self, sink: voice_recv.AudioSink, error: Optional[Exception] = None):
    ...
    async def _process_user_audio(self, user_id: int, current_time: float):
        ...
        if audio_chunks:
            processed_audio = self._process_audio_chunks(audio_chunks, user_id)
            if processed_audio:
    ...
    def _update_speaker_priorities(self, current_time: float):
        ...
        if self.speaker_priority:
            self.primary_speaker = max(self.speaker_priority.keys(), 
                                     key=lambda uid: self.speaker_priority[uid])
        else:
            ...
            if self.mix_multiple_speakers:
                ...
                for user_id in self.speaking_users:
                    if user_id in self.audio_buffer:
                        ...
                        if chunks:
                            processed = self._process_audio_chunks(chunks, user_id)
                            if processed:
                ...
                if self.primary_speaker and self.primary_speaker in self.audio_buffer:
                    ...
                    if chunks:
                        processed = self._process_audio_chunks(chunks, self.primary_speaker)
                        if processed:
    ...
    def _update_performance_stats(self, current_time: float):
        ...
        if current_time - self.last_stats_time > 30:  # Log stats every 30 seconds
            ...
            if total_frames > 0:
                ...
                log.info(f"Audio processing stats: {self.frames_processed} processed, "
                        f"{self.frames_dropped} dropped ({success_rate:.1f}% success rate)")
    ...
    def set_speaker_mixing(self, mix_multiple: bool):
        ...
        log.info(f"Multiple speaker mixing {'enabled' if mix_multiple else 'disabled'}")
...
class EnhancedAudioSink(voice_recv.AudioSink):
    """Enhanced audio sink with better error handling and user tracking."""
    
    def __init__(self, voice_capture: VoiceCapture):
        super().__init__()
        self.voice_capture = voice_capture
        
    def wants_opus(self) -> bool:
        """We want decoded PCM audio."""
        return False
    ...
    def write(self, user_id: int, data: bytes):
        ...
        try:
            if user_id and data:
                self.voice_capture.add_audio_data(user_id, data)
        except Exception as e:
            log.error(f"Error writing audio data for user {user_id}: {e}")
            
    def cleanup(self):
        """Clean up sink resources."""
        try:
            log.debug("Audio sink cleanup completed")
        except Exception as e:
